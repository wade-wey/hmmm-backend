...
class SubjectsDirectorysService extends Service {
  ...
  async find(entity, sort, page, pagesize) {
    ...
    if (entity.subjectID !== undefined) {
      parameterWhere.push(
        this.app.mysql.format('dir.subjectID = ?', `${entity.subjectID}`)
      )
    }
	...
	const qArr = await Promise.all(
		result.map(item=>this.app.mysql.query(`select count(*) as count from hm_questions where catalogID = ${item.id}`))
	)
    return {
      counts: count,
      pagesize,
  ...
  // 简单列表
  async simple(label,subjectID, sort) {
    // WHERE
    let parameterWhere = []
	if (subjectID !== undefined){
	  parameterWhere.push(this.app.mysql.format('subjectID = ?', `${subjectID}`))
	}
    if (label !== undefined) {
      parameterWhere.push(this.app.mysql.format('directoryName like ?', `%${label}%`))
    }
    const strWhere = SqlUtil.getSqlStringWhere(parameterWhere)
