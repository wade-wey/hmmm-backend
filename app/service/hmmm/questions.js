...
class QuestionsService extends Service {
  ...
  async find(entity, sort, page, pagesize) {
    ...
    // WHERE
    let parameterWhere = []
	parameterWhere.push(this.app.mysql.format('q.isChoice = ?', 0))
    // 	keyword 关键字
  ...
  async find_choice(entity, sort, page, pagesize) {
    ...
    // WHERE
    let parameterWhere = []
    parameterWhere.push(this.app.mysql.format('q.isChoice = ?', 1))
    // 	keyword 关键字
    if (entity.keyword !== undefined) {
      parameterWhere.push(
        this.app.mysql.format('q.question like ?', `%${entity.keyword}%`)
    ...
    // chkState 审核状态
    if (entity.chkState !== undefined) {
       parameterWhere.push(
         this.app.mysql.format('q.chkState = ?', `${entity.chkState}`)
       )
    }
    const strWhere = SqlUtil.getSqlStringWhere(parameterWhere)
    // ORDER
  ...
  async read(id) {
    ...
    const subject = await this.app.mysql.select('hm_subjects', {
		where: { id: result.subjectID }
    })
	result.subjectName = subject[0].subjectName
    const directory = await this.app.mysql.select('hm_subjects_directorys', {
		where: { id: result.catalogID }
    })
	result.directoryName = directory[0].directoryName
