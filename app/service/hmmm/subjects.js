...
class SubjectsService extends Service {
  ...
  async find(entity, sort, page, pagesize) {
	...
	const qArr = await Promise.all(
		result.map(item=>this.app.mysql.query(`select count(*) as count from hm_questions where subjectID = ${item.id}`))
	)
	const dArr = await Promise.all(
		result.map(item=>this.app.mysql.query(`select count(*) as count from hm_subjects_directorys where subjectID = ${item.id}`))
	)
	const tArr = await Promise.all(
		result.map(item=>this.app.mysql.query(`select count(*) as count from hm_subjects_tags where subjectID = ${item.id}`))
	)
    return {
      counts: count,
      pagesize,
      ...
      items:result.map((item,i)=>{
        item.totals = qArr[i][0].count
		item.twoLevelDirectory = dArr[i][0].count
		item.tags = tArr[i][0].count
		return item
	  })
    }
  }
